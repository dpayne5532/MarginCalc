...
# Admin Values stored in a dictionary instead of a database
admin_values = {
    'W2_VALUE': 1.2,
    'C2C_VALUE': 1.1,
    'ARIBA_VALUE': 5.0,
    'VENDORPASS_VALUE': 4.5,
    'FIELDGLASS_VALUE': 6.0,
    'ORACLE_VALUE': 5.5,
    'VMS_VALUE': 4.0,
    'BEELINE_VALUE': 3.5,
    'HAYS_VALUE': 4.8,
...
def open_admin_panel():
    ...
    def update_values():
        ...
        try:
            ...
            admin_values['W2_VALUE'] = W2_VALUE
            C2C_VALUE = float(c2c_entry.get())
            admin_values['C2C_VALUE'] = C2C_VALUE
            ARIBA_VALUE = float(ariba_entry.get())
            admin_values['ARIBA_VALUE'] = ARIBA_VALUE
            VENDORPASS_VALUE = float(vendorpass_entry.get())
            admin_values['VENDORPASS_VALUE'] = VENDORPASS_VALUE
            FIELDGLASS_VALUE = float(fieldglass_entry.get())
            admin_values['FIELDGLASS_VALUE'] = FIELDGLASS_VALUE
            ORACLE_VALUE = float(oracle_entry.get())
            admin_values['ORACLE_VALUE'] = ORACLE_VALUE
            VMS_VALUE = float(vms_entry.get())
            admin_values['VMS_VALUE'] = VMS_VALUE
            BEELINE_VALUE = float(beeline_entry.get())
            admin_values['BEELINE_VALUE'] = BEELINE_VALUE
            HAYS_VALUE = float(hays_entry.get())
            admin_values['HAYS_VALUE'] = HAYS_VALUE
            PRIORITY_VALUE = float(priority_entry.get())
            admin_values['PRIORITY_VALUE'] = PRIORITY_VALUE
